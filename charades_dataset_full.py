...
def load_rgb_frames(image_dir, vid, start, num):
  ...
  for i in range(start, start+num):
    img = cv2.imread(os.path.join(image_dir, vid, vid+'_'+str(i).zfill(5)+'.jpg'))[:, :, [2, 1, 0]]
    w,h,c = img.shape
    if w < 226 or h < 226:
        d = 226.-min(w,h)
        sc = 1+d/min(w,h)
...
def make_dataset(split_file, split, root, mode, num_classes=157):
    ...
    for vid in videos:
        ...
        dataset.append((vid, 0, 0, num_frames))
        i += 1
...
class Charades(data_utl.Dataset):
    ...
    def __init__(self, split_file, split, root, mode, transforms=None, save_dir='i3d-features-ca/Test', num=0):
    ...
    def __getitem__(self, index):
        ...
        return video_to_tensor(imgs), 0, vid
